...
if __name__ == '__main__':
    ...
    parser.add_argument('--mapping', default= "{0:'绿',1:'黄'}" , type=str, help="label mapping")
    ...
    template = '*cls**mask*黑。*sent_0**sep+*\n'
    print(f'##### {template} #####')
...
#     model = torch.nn.DataParallel(BertForMaskedLM.from_pretrained(path))
    ...
    for epoch in range(epochs):
        ...
        saved_model_path = f'{args.output_dir}/{args.task}/{seed}_{epoch}'
        ...
        if epoch == (args.epochs-1):
            ...
            with open(f'{args.result_output_dir}{args.task}_{seed}.txt','a') as out_file:
                out_file.write(f"epoch_{epoch}: {acc}\n")
